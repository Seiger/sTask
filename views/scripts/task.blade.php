<script>
     ...
     *
     * @param {string} key - Widget identifier for progress bar ID
    ...
    async function uploadFile(file, root, widgetIdentifier, uploadUrl) {
        try {
            // Get server limits
            const result = await callApi("{{route('sTask.serverLimits')}}", null, 'GET');
            ...
            if (file.size > maxSize) {
                widgetLogLine(root, `**{{__('sTask::global.file_too_large')}}** (max: ${niceSize(maxSize)})`, 'error');
            ...
            if (file.size > singleUploadLimit) {
                ...
                widgetLogLine(root, `{{__('sTask::global.chunk_upload')}}: ${totalChunks} {{__('sTask::global.chunks')}}`);
                ...
                for (let i = 0; i < totalChunks; i++) {
                    ...
                    if (!result.success) {
                        widgetProgressBar(widgetIdentifier, 0);
                        throw new Error(result.message || '{{__('sTask::global.upload_failed')}}');
                    }

                    // Update progress
                    const progress = Math.round(((i + 1) / totalChunks) * 100);
                    widgetProgressBar(widgetIdentifier, progress, `{{__('sTask::global.uploading_file')}} ${progress}%`);
            ...
            widgetLogLine(root, '**{{__('sTask::global.upload_failed')}}:** _' + error.message + '_', 'error');
            enableButtons(widgetIdentifier);
            widgetProgressBar(widgetIdentifier, 0);
        }
    }
