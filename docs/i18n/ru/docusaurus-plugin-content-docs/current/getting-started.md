...
### Шаг 6: Обнаружьте воркеров

```console
php artisan stask:discover-workers
```

Это сканирует ваши установленные пакеты на наличие воркеров, которые реализуют `TaskInterface`, и регистрирует их автоматически.

> **Примечание:** Пакет автоматически интегрируется с интерфейсом менеджера Evolution CMS. Дополнительная конфигурация не требуется!
...
- **Вкладка Воркеры** - Управление воркерами и обнаружение
- **Вкладка Статистика** - Детальная аналитика
...
- Исследуйте [API справочник](./api.md) для расширенного использования
- Просмотрите [Примеры](./examples.md) для типичных случаев использования
