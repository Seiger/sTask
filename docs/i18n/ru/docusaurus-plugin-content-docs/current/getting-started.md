...
**Права доступа заполняются автоматически** после миграций. Если автоматическое заполнение не сработает, выполните:
```console
php artisan stask:seed-permissions
```

Это создаст группу прав `stask` и необходимые права доступа для контроля доступа к интерфейсу sTask.
