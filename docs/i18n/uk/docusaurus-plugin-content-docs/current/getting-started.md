...
Також створюється право доступу `stask_access` для контролю доступу до інтерфейсу sTask.

### Крок 6: Налаштування Обробника Задач

Команда обробника задач автоматично обробляє очікуючі задачі. Додайте її до cron або планувальника задач:

```console
php artisan stask:worker
```

Для безперервної обробки додайте до crontab:
...
- Повинна запускатись через cron щохвилини для безперервної обробки
