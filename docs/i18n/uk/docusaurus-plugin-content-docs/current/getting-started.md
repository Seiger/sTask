...
**Права доступу заповнюються автоматично** після міграцій. Якщо автоматичне заповнення не спрацює, виконайте:
```console
php artisan stask:seed-permissions
```

Це створює групу прав `stask` та необхідні права доступу для контролю доступу до інтерфейсу sTask.
